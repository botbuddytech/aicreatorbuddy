...
function runnerError(error: CursorRunnerError) {
  ...
  if (error.code === "invalid-output") {
    return json({ error: "Cursor returned an invalid low-effort report." }, 502);
  }
  return json({ error: "Cursor could not analyze the script." }, 502);
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (raw.length > MAX_REQUEST_BYTES) return json({ error: "Script data is too large." }, 413);
    let body: unknown;
    try {
      body = JSON.parse(raw);
    } catch {
    ...
    const input = parseCursorScriptLowEffortRequest(body);
    if (!input) return json({ error: "Invalid script or reference data." }, 400);
    const prompts = await getEffectiveCursorPrompts(user.id);
    return json(
      await checkScriptLowEffortWithCursor(
        prompts.scriptLowEffort,
        input,
        request.signal,
      ),
    );
    ...
    console.error("[cursor-script-low-effort] failed", error);
    return json({ error: "Cursor could not analyze the script." }, 500);
  }
}
